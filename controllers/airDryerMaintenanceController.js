...
const COMPONENT_LABELS = {
  ...
  moistureSeparator: "Moisture Separator",
  coolant: "Coolant",
  refrigerant: "Refrigerant Level / Pressure",
};
...
const nextDateFor = (component, componentConfig) => {
  ...
  if (componentConfig.key === "refrigerant") return component.nextServiceDate || undefined;
...
// @desc    Create an air dryer maintenance record
// @route   POST /api/air-dryer-maintenance
// @access  Employee (assigned), General Manager, Owner, Admin
const createAirDryerMaintenance = asyncHandler(async (req, res) => {
  ...
  const record = await AirDryerMaintenance.create({
    ...req.body,
    performedBy,
...
// @desc    List air dryer maintenance records
// @route   GET /api/air-dryer-maintenance
// @access  Protected
const getAirDryerMaintenanceRecords = asyncHandler(async (req, res) => {
...
// @desc    Get single air dryer maintenance record
// @route   GET /api/air-dryer-maintenance/:id
// @access  Scoped
const getAirDryerMaintenanceById = asyncHandler(async (req, res) => {
  const record = await AirDryerMaintenance.findById(req.params.id)
    .populate("machine", "machineName machineNumber")
    .populate("performedBy", "name employeeId");
  if (!record) {
    res.status(404);
    throw new Error("Air dryer maintenance record not found");
  }
  await assertRecordAccess(req, res, record);
  res.json({ success: true, data: record });
});

// @desc    Update an air dryer maintenance record
// @route   PUT /api/air-dryer-maintenance/:id
// @access  Employee (own), General Manager, Owner, Admin
const updateAirDryerMaintenance = asyncHandler(async (req, res) => {
  const record = await AirDryerMaintenance.findById(req.params.id);
  if (!record) {
    res.status(404);
    throw new Error("Air dryer maintenance record not found");
...
// @desc    Delete an air dryer maintenance record
// @route   DELETE /api/air-dryer-maintenance/:id
// @access  Admin, Owner
const deleteAirDryerMaintenance = asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, message: "Air dryer maintenance record deleted" });
});

// @desc    Per-component maintenance schedule for a machine
// @route   GET /api/air-dryer-maintenance/schedule?machine=...
// @access  Protected
const getAirDryerSchedule = asyncHandler(async (req, res) => {
  ...
  const records = await AirDryerMaintenance.find({ machine }).sort({ maintenanceDate: -1 });
...
// @route   GET /api/air-dryer-maintenance/due/today
// @access  Scheduler
const getDueAirDryerMaintenance = asyncHandler(async (req, res) => {
  const endOfToday = new Date();
  endOfToday.setHours(23, 59, 59, 999);
  const due = await AirDryerMaintenance.find({
...
// @route   PATCH /api/air-dryer-maintenance/:id/mark-reminder-sent
// @access  Scheduler
const markAirDryerReminderSent = asyncHandler(async (req, res) => {
  const record = await AirDryerMaintenance.findById(req.params.id);
  if (!record) {
    res.status(404);
    throw new Error("Air dryer maintenance record not found");
